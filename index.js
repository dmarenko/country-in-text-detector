  ...
  for (var i = 0; i < arr1.length; i++) {
    for (var j = lastJ; j < arr2.length; j++) {
      if (
        ...
        (arr1[i].startsWith(arr2[j]) || arr1[i].endsWith(arr2[j]))
      ) {
        arr2.splice(j, 1);
        lastJ = j;
        i++;
      ...
      if (
        arr1[i].length < arr2[j].length &&
        (arr2[j].startsWith(arr1[i]) || arr2[j].endsWith(arr1[i]))
      ) {
        arr1.splice(i, 1);
        lastJ = j;
        i++;
...
export function detect(str) {
  ...
  for (var i = 0; i < COUNTRIES.length; i++) {
    if (
      typeof COUNTRIES[i].string_match === "undefined" ||
      COUNTRIES[i].string_match === null
    ) {
      continue;
    }
